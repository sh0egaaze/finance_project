...
const shortenName = (name: string, maxLen: number = 12) => {
...
export default function Analytics() {
  ...
  const cardClass = "bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6";
  const inputClass = "px-2 sm:px-3 py-1.5 sm:py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 text-xs sm:text-sm bg-white dark:bg-gray-700 dark:text-white";
  const switchBtn = "p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors";
  const titleClass = "text-base sm:text-lg font-semibold text-gray-800 dark:text-white";
  ...
  const incomeByCategory = (data.income_by_category || []).map((item) => ({
    ...item,
    fill: item.color || '#22c55e',
  }));
  const totalSpending = spendingByCategory.reduce((a, b) => a + b.amount, 0);
  const totalIncomeCalc = incomeByCategory.reduce((a, b) => a + b.amount, 0);
  ...
  if (spendingByCategory.length === 0 && incomeByCategory.length === 0) {
    return (
      <div className="space-y-4 sm:space-y-6">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white">Аналитика</h2>
        <div className={`${cardClass} p-12 text-center`}>
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
        <div className={cardClass}>
          <p className="text-gray-500 dark:text-gray-400 text-xs sm:text-sm">Доходы</p>
          <p className="text-lg sm:text-2xl font-bold text-green-600 dark:text-green-400 truncate">{formatCurrency(data.total_income || 0)}</p>
        </div>
        <div className={cardClass}>
          <p className="text-gray-500 dark:text-gray-400 text-xs sm:text-sm">Расходы</p>
          <p className="text-lg sm:text-2xl font-bold text-red-600 dark:text-red-400 truncate">{formatCurrency(totalSpending)}</p>
        </div>
        <div className={cardClass}>
          <p className="text-gray-500 dark:text-gray-400 text-xs sm:text-sm">Баланс</p>
          <p className={`text-lg sm:text-2xl font-bold truncate ${(data.total_income || 0) - totalSpending >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400'}`}>
            {formatCurrency((data.total_income || 0) - totalSpending)}
          </p>
        </div>
        <div className={cardClass}>
          <p className="text-gray-500 dark:text-gray-400 text-xs sm:text-sm">Топ категория</p>
          <p className="text-lg sm:text-2xl font-bold text-gray-800 dark:text-white truncate">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={currentChartData}>
                ...
                <XAxis
                  ...
                  tickFormatter={(name) => shortenName(name, 10)}
                  interval={0}
                />
                ...
                <Bar dataKey="amount" radius={[4, 4, 0, 0]}>
                  {currentChartData.map((entry, index: number) => (
        ...
        <div className={cardClass}>
          ...
          {currentChartData.length > 0 ? (
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="flex-1 space-y-2 max-h-[200px] overflow-y-auto w-full sm:w-auto">
                {currentChartData.map((cat) => {
                  ...
                  return (
                    <div key={cat.category_id || cat.name} className="flex items-center justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: cat.fill || cat.color || (showIncome ? '#22c55e' : '#6B7280') }} />
                        <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 truncate">{cat.name}</span>
                      </div>
                      <span className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400 ml-2 shrink-0">{percentage.toFixed(1)}%</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className={cardClass}>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={data.spending_trend}>
              ...
              />
              <Line type="monotone" dataKey="income" stroke="#10B981" name="Доходы" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
              <Line type="monotone" dataKey="expense" stroke="#EF4444" name="Расходы" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className={cardClass}>
        ...
        {currentChartData.length > 0 ? (
          <div className="space-y-2 sm:space-y-3">
            {[...currentChartData].sort((a, b) => b.amount - a.amount).map((item) => {
              const percentage = currentTotal > 0 ? (item.amount / currentTotal) * 100 : 0;
              return (
                <div key={item.category_id || item.name} className="flex items-center gap-2 sm:gap-4">
                  <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full shrink-0" style={{ backgroundColor: item.fill || item.color || (showIncome ? '#22c55e' : '#6B7280') }} />
                  <span className="flex-1 text-sm sm:text-base text-gray-700 dark:text-gray-300 truncate">{item.name}</span>
                  <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 shrink-0">{percentage.toFixed(1)}%</span>
                  <span className="font-semibold text-sm sm:text-base text-gray-800 dark:text-white w-20 sm:w-28 text-right shrink-0">{formatCurrency(item.amount)}</span>
                </div>
              );
            })}
          </div>
